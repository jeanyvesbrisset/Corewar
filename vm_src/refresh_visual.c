...
/*   Updated: 2019/07/30 16:52:04 by maginist         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	refresh_cycle_to_die(t_core *core)
{
	...
	mvwprintw(core->visu->hud, 26 + (i * 4), 20 + ft_strlen(core->visu->str)
    , "    ");
	ft_strdel(&(core->visu->str));
}
...
void	refresh_live_ctd(t_core *core)
{
	...
	while (i < core->champ_nb)
	{
		...
		while (j - core->visu->live_bd[i] < (cur->live_by_ctd * 100) / core->nbr_live)
			mvwprintw(core->visu->hud, 23 + (i * 4), 6 + j++, "-");
		if ((((cur->live_by_ctd * 1000) / core->nbr_live) % 10) >= 5
			&& j < 100)
			mvwprintw(core->visu->hud, 23 + (i * 4), 6 + j++, "-");
...
void	visual_every_cycle(t_core *core)
{
	...
	while (pr)
	{
		mvwchgat(core->visu->arena, pr->pc / 64, pr->pc % 64, 2, A_REVERSE, 0
		, 0);
		pr = pr->next;
	}
...
{
	mvprintw(core->visu->hud, 15, 18, core->sum_process);
	wrefresh(core->visu->hud);
}